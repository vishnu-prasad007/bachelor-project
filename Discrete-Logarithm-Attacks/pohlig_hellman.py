...
def pohlig_hellman(g: int, y: int, n: int, factors: int, powers: int, verify: bool = False) -> int:
    """ 
    Works best when factors of n-1 are small
    ...
    # Using the Chinese remainder theorem, find x (mod n-1).
...
def __pohlig_hellman_helper(g: int, y: int, n: int, p: int, e: int, verify: bool = False) -> int:
    ...
    """
    if verify:
        assert (n-1) % pow(p, e) == 0, "Incorrect input to __pohlig_hellman_helper"
    ...
    # The loop iterates, computing x j with each pass, and constructing the response x appropriately.
    for j in range(e):
        # This makes h = g ^ (x_i * ((n-1)/p)) (mod n)
        # h = pow(y, (n-1)//pow(p, (j+1)), n)
        h = pow(y, (n-1)//(p_power * p), n)
        # h is the x_jth power of gamma
        # Now, we use the Discrete Log technique to determine x_j
        # When p > 2 we use BSGS when p = 2 we use Brute
        # BSGS takes sqrt(p) time and Brute takes O(1) time as x_j in that case is 0/1.
        if p > 2:
            # We limit BSGS to only search till p because we know that x_j < p.
            x_j = bsgs(gamma, h, n, p)
        else:
            x_j = brute(gamma, h, n)
        if x_j < 0:
            print("Either there isn't a solution for x, or this is a case where the Pohlig Hellman algorithm fails.")
            exit()

        # Update x by adding x_j * (p^j)
        curr_exp = p_power * x_j
        x += curr_exp
