...
def extended_euclidean_algorithm(a: int, b: int) -> (int, int, int):
    # The algorithm has been implemented from Pseudocode from Wikipedia article

    # s captures the Bezout's coefficient of a, and t does it for b for any number
    # At any point we store the Bezout's representation, of the last two values in the algorithm we are considering
...
def solve_linear_congurence(a: int, b: int, m: int):
    ...
    :returns: A list of solutions is returned if the equation can be solved.
              Otherwise returns -1.
    """
...
def chinese_remainder(x : int, m: int, verify: bool = False) -> int:
    """
...
def is_prime(n: int) -> bool:
    ...
    # Therefore, we begin with 5 and increase the divisor by 6, the number is composite if the divider divides it. 
